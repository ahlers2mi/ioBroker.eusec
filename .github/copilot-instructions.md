...
8. [Adapter-Specific Coding Standards](#adapter-specific-coding-standards)
...
**Adapter Name:** eusec (ioBroker.eusec)

**Primary Function:** Integration with Eufy Security cameras, doorbells, and security stations. This adapter connects ioBroker to Eufy Security devices by communicating with Eufy cloud servers and establishing local/remote P2P connections to stations.

**Key Features:**
- Cloud connection to Eufy account for device discovery and management
- Local/remote P2P connection support to Eufy stations and devices
- Camera livestream access and control
- Security event monitoring (motion detection, doorbell presses, etc.)
- Device state management and control (arm/disarm, settings, etc.)

**Primary Dependencies:**
- `eufy-security-client`: Core library for Eufy device communication (v3.1.1)
- `@bropat/fluent-ffmpeg`: Video stream processing
- `ffmpeg-for-homebridge`: FFmpeg binaries for video operations
- `go2rtc-static`: Real-time communication for video streaming

**Important Notes:**
- Cloud connection is ALWAYS required as a prerequisite, even for local P2P connections
- The adapter is NOT affiliated with Anker/Eufy - it's a community project
- Uses TypeScript for development (transpiled to JavaScript in build/)
- Requires Node.js >= 20.0.0
- Maintained by iobroker-community-adapters organization (originally by bropat)

**Configuration Requirements:**
- Eufy Cloud credentials (username/password)
- Station and device discovery through cloud API
- Optional: P2P connection settings for local communication
- Stream configuration for video access
...
- Transpiled JavaScript output goes to build/ directory
- Use TypeScript strict mode and proper typing
- Follow existing code patterns in src/main.ts

### eufy-security-client Integration
- Use the eufy-security-client library for all Eufy device interactions
- Handle P2P connection lifecycle properly (connect, disconnect, cleanup)
- Implement proper error handling for cloud API failures
- Respect rate limiting and connection state management

### State Management
- Follow ioBroker state naming conventions
- Use proper state roles and types as defined in io-package.json
- Implement state acknowledgment correctly
- Clean up states properly on adapter shutdown
